                ...
                if not _sd_has_evidence:
                    try:
                        ...
                        if isinstance(_sd_sources, str):
                            _sd_sources = json.loads(_sd_sources) if _sd_sources.strip() else []
        ...
        try:
            ...
            if sd_result or _is_known_hit:
                if sd_result:
                    ...
                    update_payload["single_sources"] = json.dumps(sd_result.get("sources", []), default=str)
                else:
                    update_payload["is_single"] = True
                    update_payload["single_confidence"] = "medium"
                    update_payload["single_confidence_score"] = 0.85
                    update_payload["single_sources"] = json.dumps([{"source": "hit_safeguard", "matched": True}])
